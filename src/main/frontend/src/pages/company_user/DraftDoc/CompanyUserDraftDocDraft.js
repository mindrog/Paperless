...
function CompanyUserDraftDocDraft() {
    ...
    const completedColumns = [
        ...
        { key: 'repo_date', label: '임시저장일', width: '15%' },
    ...
    useEffect(() => {
        const fetchDocRequest = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:8080/api/getdraftassavelist', {
                    headers: {
                        'Authorization': token
                    }
                });
...
export default CompanyUserDraftDocDraft;
