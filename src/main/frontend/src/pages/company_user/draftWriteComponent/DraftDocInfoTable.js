    ...
    <Table bordered size="sm" className={styles.docInfo}>
      <tbody>
        ...
        <tr>
          ...
          <td className={styles.docValue}>{department}</td>
        </tr>
        <tr>
          <td className={styles.docKey}>소 &nbsp;속 &nbsp;팀</td>
          <td className={styles.docValue}>{team}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td className={styles.docValue}>{reporter}</td>
        </tr>
        <tr>
          ...
          <td className={styles.docValue}>
            <Form.Control
              type="date"
              value={repoStartTime}
              className={`${styles.inputFormDate} ${dateError && repoStartTime > repoEndTime ? 'is-invalid' : ''}`}
              onChange={(e) => handleStartDateChange(e.target.value)}
              isInvalid={!!dateError && repoStartTime && repoStartTime > repoEndTime}
            />
            {dateError && repoStartTime > repoEndTime && (
        ...
        <tr>
          ...
          <td className={styles.docValue}>
            <Form.Control
              type="date"
              value={repoEndTime}
              className={`${styles.inputFormDate} ${dateError && repoEndTime < repoStartTime ? 'is-invalid' : ''}`}
              onChange={(e) => handleEndDateChange(e.target.value)}
              isInvalid={!!dateError && repoEndTime && repoEndTime < repoStartTime}
            />
            {dateError && repoEndTime < repoStartTime && (
              <Form.Text className="text-danger">
                {dateError}
              </Form.Text>
            )}
        ...
        <tr>
          ...
          <td className={styles.docValue}>{reportStatus || '신청'}</td>
        </tr>
      </tbody>
    </Table>
  );
